...
def buildParserFromArgs():
    ...
    argParser.add_argument("--parser", help="which parser [GRAPH|TRANS|MTL]", required=True)
...
def buildAnalysisTool(args, reprBuilder):
...
if __name__ == "__main__":
    ...
    normalizer =  utils.buildNormalizer(utils.parseBoolean(args.normalize))
            
    parser.load(args.model)
        
    testData = utils.readSentences(args.test, tokenBuilder, normalizer)
    analysisTool = buildAnalysisTool(args, parser.getReprBuilder())
