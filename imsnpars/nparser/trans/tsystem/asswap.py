...
class Swap(tdatatypes.Transition):
    ...
    def isValid(self, state):
        ...
        stackTop = state.getStackElem(0)
        stackSecond = state.getStackElem(1)
        return stackSecond != tdatatypes.State.ROOT and stackTop > stackSecond
    
class ArcStandardWithSwap(tdatatypes.TransSystem):
...
class MPCBuilder(object):
    def __init__(self):
        self.__noSwap = arcstandard.ArcStandard()
        self.__noSwapOracle = arcstandard.ArcStandardStaticOracle(self.__noSwap)
        
    def buildMPCs(self, tree):
        noSwapTrans = oracle.buildStaticCorrectTransitions(tree, self.__noSwap, self.__noSwapOracle)
        projectiveForest = tdatatypes.buildTreeFromTransitions(self.__noSwap, tree.nrOfTokens(), noSwapTrans)
