# Taken from https://github.com/stanfordnlp/stanfordnlp/blob/master/stanfordnlp/models/common/chuliu_edmonds.py
# Adapted from Tim's code here: https://github.com/tdozat/Parser-v3/blob/master/scripts/chuliu_edmonds.py
# Adapted from UUParser https://github.com/UppsalaNLP/uuparser/blob/master/barchybrid/src/chuliu_edmonds.py
...
class ChuLiuEdmonds(gdatatypes.MaximumSpanningTreeAlgorithm):
    ...
    def findMSTHeads(self, scores, imposeOneRoot):
        weights = np.array(scores.scores)
        trWeights = np.transpose(weights)
        
        if imposeOneRoot:
            fastTree = chuliu_edmonds_one_root(trWeights)
        else:
            fastTree = chuliu_edmonds(trWeights)
...
def chuliu_edmonds_one_root(scores):
    ...
    if len(roots_to_try) == 1:
        return tree
    #else:
    #    print("Selecting from", len(roots_to_try))
