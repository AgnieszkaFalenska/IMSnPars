...
class ArcHybridDynamicOracle(oracle.Oracle):
    ...
    def _calculateLeftCost(self, state, tree):
        ...
        if state.stack.empty() or state.buffer.empty() or state.stack.top() == State.ROOT:
            leftCost = np.inf
        else:
            # s0 children
            s0 = state.stack.top()
    ...
    def _calculateRightCost(self, state, tree):
        ...
        if state.stack.length() < 2 or state.stack.top() == State.ROOT:
            rightCost = np.inf
        else:
            # s0 children
            s0 = state.stack.top()
