...
class LeftArc(Transition):
    ...
    def isValid(self, state):
        ...
        return state.stack.length() == 1 or state.buffer.head() != State.ROOT
...
class Shift(Transition):
    ...
    def isValid(self, state):
        return not state.buffer.empty() and state.buffer.head() != State.ROOT
...
class ArcHybridWithSwap(TransSystem):
    ...
    def isFinal(self, state):
        return state.stack.empty() and state.buffer.length() == 1 and state.buffer.head() == State.ROOT
    
    def getNrOfTransitions(self):
        return 4
    
    def initialState(self, nrOfTokens):
        return State(nrOfTokens, False)
